...
### Phase 4: Sidecar 60fps Native UI 렌더러 (C/LVGL 또는 Rust)
- [ ] **크로스 컴파일 환경 구축**:
  - ARMv5TE(`arm-unknown-linux-musleabi`) 빌드 환경 (Nix derivation 또는 툴체인)
- [ ] **초경량 프레임버퍼 드라이버**:
  - `/dev/fb0` 더블 버퍼링 기반 60fps 드라이버 초기화
- [ ] **LVGL 기반 UI 컴포넌트 개발**:
  - **시계 모드**: 아날로그 플립/디지털 시계 (부드러운 초침 회전)
  - **시스템 모니터링**: CPU/GPU 온도, RAM, 스토리지 (부드러운 게이지 차오름)
  - **홈랩 모니터링**: Docker 컨테이너 실행 수, Tailscale 상태, 실시간 트래픽 그래프
- [ ] **1Hz 텔레메트리 파서**:
  - 본체가 1초마다 보내는 가벼운 수치 데이터를 수신하여 UI 게이지에 애니메이션 보간(스무딩) 적용

---

### Phase 5: 터치 스트림덱 / 매크로 패드 연동
- [ ] **터치 입력 파이프라인**:
  - `/dev/input/event0` 및 `tslib` 연동으로 터치 좌표 및 제스처(탭, 스와이프) 감지
- [ ] **업링크 프로토콜 구현**:
  - 화면 터치 시 시리얼/SSH 링크로 본체에 이벤트 패킷 전송 (예: `BUTTON:1:PRESS`)
- [ ] **본체 액션 매퍼 개발**:
  - 볼륨 조절 / 음소거
  - 특정 Docker 컨테이너 / 시스템 서비스 재시작
  - 홈랩 백업 스크립트 실행
  - 홈 오토메이션(조명/플러그) 제어
