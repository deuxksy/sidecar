...
AM-02 전면 디스플레이(subscreen)에 Windows AYASPACE와 동일한 수준의 시스템 정보(시계, CPU/GPU 온도, 팬 속도, 부하)를 NixOS에서 표시한다.
...
즉 **커널 드라이버는 불필요**하다. subscreen은 이미 독립 실행 중이며, 호스트 쪽에서 시리얼 프로토콜로 데이터를 보내주는 사용자 공간 데몬만 있으면 된다.
...
1. **`docs/protocol.md`** — 역설계 산출물. 캡처된 AYASPACE↔subscreen 프레임 구조 문서 (헤더, 필드 레이아웃, 체크섬, 전송 주기)
2. **`am02-subscreend`** — 메인 데몬. 수집 → 인코딩 → 전송 루프 (1초 주기). 설정은 표시 항목 ON/OFF 정도로 최소화
3. **프로토콜 인코더 모듈** — 프레임 직렬화만 담당. 코어 루프에서 분리하여 단위 테스트 대상으로 격리
4. **`nix/module.nix`** — NixOS 서비스 정의 (`systemd.services`) + udev 규칙 (by-id 심링크)
...
필요한 것만 (불가능한 시나리오 과대처리 금지):

- 시리얼 장치 부재/지연 → 지수 백오프 재시도 (subscreen이 메인 OS보다 늦게 준비되는 케이스)
- 화면 미갱신 감지 시 로그 1줄 기록 — 자동 복구 로직 없음

## 검증 기준

| 단계 | Verify |
| :--- | :--- |
| Phase 0 | 장치 노드와 VID/PID가 문서화됨 |
| Phase 1 | 캡처 덤프에서 반복 가능한 프레임 패턴 식별됨 |
| Phase 2 | 인코더 단위 테스트: 캡처된 실제 프레임을 코드로 재생성 → 바이트 단위 일치 |
| Phase 3 | 실기: 부팅 후 subscreen에 시계+온도 표시, 1초 주기 갱신 (최종 성공 기준) |
...
| Linux에서 장치가 tty로 열리지 않음 (vendor-specific USB) | 저 | Phase 0에서 즉시 판명. 필요시 usbserial vendor 파라미터로 대응 |
